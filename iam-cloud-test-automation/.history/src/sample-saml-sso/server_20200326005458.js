...
app.get('/app', redirectToLogin, (req, res, next) => {
	
    if (req.isAuthenticated() || userProfile != null) {
        userName =JSON.stringify(userProfile);
        
      matches = userName.match(/(?:\"http)(.*)/g);
        // var newLine = matches.split(",").join("\n");
        // console.log("^&^&^^&^"+matches);
		res.render('home', {
		title: 'Express Web Application',
        heading: 'Logged-In to Express Web Application',
        
        user: userProfile.nameID,
        claim: matches
...
app.post(
	...
	(req, res) => {
		...
		fs.writeFile('../iam-e2e-test/cypress/fixtures/saml_sso.txt', decoded, function (err,data) {
			if (err) {
